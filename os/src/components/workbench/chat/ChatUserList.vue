...
<template>
    <el-collapse v-model="store.activeNames">
        <el-collapse-item name="1">
            <template #title>
                <span v-if="store.onlineUserList.length > 0" class="title">在线（{{ store.onlineUserList.length }}）</span>
                <span v-else class="title">在线</span>
            </template>
            <div v-if="store.onlineUserList.length > 0">
                <div v-for="item in store.onlineUserList" :key="item.id">
                    <div class="list-item" @click="store.getSessionInfo(item.chatId, 'user')" :style="{
                        backgroundColor:
        ...
        <el-collapse-item name="2">
            ...
            <div class="tree-container" v-if="data.length > 0">
                <el-tree :data="data" node-key="dept_id" :props="{ label: 'label', children: 'children' }"
                    @node-click="handleNodeClick" :default-expand-all="false">
                </el-tree>
            </div>
        ...
        <el-collapse-item name="3">
            <template #title>
                <span v-if="store.groupList.length > 0" class="title">群组（{{ store.groups.length }}）</span>
                <span v-else class="title">群组</span>
            </template>
            <div v-if="store.groupList.length > 0">
                <div v-for="group in store.groups" :key="group.id">
                    <div class="list-item" @click="store.getSessionInfo(group.id, 'group')" :style="{
                        backgroundColor:
                        ...
                        <el-row>
                            <el-col :span="6">
                                <el-avatar shape="square" :size="40" class="avatar">群</el-avatar>
                            </el-col>
                            <el-col :span="18">
