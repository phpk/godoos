...
import {getUrl} from './hack';
import tinycolor from 'tinycolor2';
import {cloneDeep, get, find} from 'lodash';
import {getDarkColor, getLightColor, themeColor,indexedColor } from './color';
...
function requestExcel(src:any, options:any) {
...
export function readExcelData(buffer:any){
    try {
            ...
            try {
                ...
                if(cell.style.numFmt){
                    if(cell.style.numFmt.endsWith('%')){
                        ...
                        let result:any = value.toFixed(precision) + '';
...
function transferArgbColor(originColor:any){
    ...
    let color:any = {};
    try {
        let argb:any = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(originColor);
...
export function transferExcelToSpreadSheet(workbook:any, options:any){
    ...
    workbook.eachSheet((sheet:any) => {
        ...
        let sheetData:any = { name: sheet.name,styles : [], rows: {},cols:{}, merges:[],media:[] };
        // 收集合并单元格信息
        ...
        (sheet._rows || []).forEach((row:any,spreadSheetRowIndex:any) =>{
            ...
            if(row.height){
                sheetData.rows[spreadSheetRowIndex].height = row.height + (options.heightOffset || 8);
            }else{
        ...
        let tempRowsKeys:any = Object.keys(sheetData.rows);
