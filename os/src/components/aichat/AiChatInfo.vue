...
import { infoMsg,successMsg } from "@/utils/msg.ts";
...
const changeInfo = async () => {
  ...
  //console.log(info)
  if (!info.title) {
    infoMsg(t("aichat.inputTitle"));
    return;
  }
  if (!info.model) {
    infoMsg(t("aichat.selectModel"));
    return;
  }
  if (!info.prompt) {
    info.prompt = "";
  ...
  if (chatStore.isEditor) {
    ...
    info.engine = modelData.engine;
    await chatStore.updateChat(info, chatStore.activeId);
    successMsg(t("aichat.editsuccess"));
  } else {
    const promptData = {
      prompt: info.prompt,
    ...
    successMsg(t("aichat.addsuccess"));
  }
