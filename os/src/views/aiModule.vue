...
import { successMsg, errMsg } from "@/utils/msg.ts";
...
async function download(saveData: any) {
  ...
  if (has) {
    errMsg(t('model.labelDown'));
    return;
  }
  if (saveData.type == "net") {
    //console.log(saveData)
    await modelStore.addModel(saveData);
    await modelStore.checkModelList(saveData)
    successMsg(t('model.labelDown'));
    return;
  }

  try {
    const downUrl = modelStore.getOllamaDownUrl()
...
async function refreshOllama() {
  try {
    ...
    successMsg(t('model.refreshSuccess'));
  } catch (error) {
    errMsg(t('model.refreshFail'));
  }
