...
async function del(item: any) {
  ...
  if (!flag) {
    errMsg(t("prompt.contDelete"))
    return
  } else {
    successMsg(t("prompt.delSuccess"))
  }
}
</script>
<template>
  <div class="assistant-main">
    ...
    <el-drawer v-model="assistantStore.showLeft" direction="ltr" style="height: 100vh" width="300px" :show-close="false"
      :with-header="false">
      <el-space direction="vertical" alignment="flex-start">
        <el-button type="primary" style="width:150px">
          {{ t('prompt.cate') }}
        </el-button>
        <el-button @click.stop="assistantStore.changeCate('all')" text>
          {{ t('prompt.all') }}
        </el-button>
        <el-button @click.stop="assistantStore.changeCate(item)" v-for="item in assistantStore.promptAction" text>
          {{ t('prompt.' + item) }}
        </el-button>
      </el-space>
    </el-drawer>
    ...
    <div style="padding: 15px">
      <el-card class="model-item" v-for="(item, key) in assistantStore.promptList" :key="key" shadow="hover"
        style="margin: 8px 0">
      ...
      <el-row justify="center" style="margin-top: 15px" v-if="assistantStore.page.pages > 1">
        <el-pagination background layout="prev, pager, next" v-model:current-page="assistantStore.page.current"
          v-model:page-size="assistantStore.page.size" :total="assistantStore.page.total"
