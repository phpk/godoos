<script setup lang="ts">
	...
	const updateProxy = () => {
		if (pwdRef.value.validate()) {
			const index = proxies.value.findIndex(
				(p) => p.id === proxyData.value.id
			);
			if (index !== -1) {
	...
	const totalPages = computed(() =>
		Math.ceil(proxies.value.length / pageSize)
	);
...
<template>
	<div>
		...
		>
			...
			<el-table-column fixed="right"  label="操作">
				<template #default="scope">
					<el-button
						size="small"
						@click="editNas(scope.row)"
						>编辑</el-button
					>
					<el-button
						size="small"
						type="danger"
						@click="deleteNas(scope.row.id)"
						>删除</el-button
					>
				</template>
			</el-table-column>
		...
		<el-dialog
			...
			:title="isEditing ? '编辑Nas客户端' : '添加Nas客户端'"
			width="400px"
		>
			<span>
				<el-form
