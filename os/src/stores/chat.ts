...
import { OnlineUserInfoType, Message} from "@/interfaces/chat.ts"
...
export const useChatStore = defineStore('chatStore', () => {
  ...
  const initChat = async () => {
    if (loginStore.userInfo.avatar == '' || loginStore.userInfo.avatar == 'undefined') {
      loginStore.userInfo.avatar = '/logo.png'
    }
  ...
  const getAllList = async () => {
    const res = await get('user/chat/user/list')
    // console.log(res);
    if (res.success) {
      groups.value = res.data.groups
      departmentList.value = res.data.users
      ...
      const transformUser = (data: any) => {
        ...
        data.forEach((dept: any) => {
          if (dept.users) {
            allUser.push(...dept.users)
          }
          transformUser(dept.sub_depts)
        })
      }
      transformUser(res.data.users)

      allUser.forEach(async (user: any) => {
        const userToProcess = {
          id: user.user_id,
  ...
  const initChatList = async () => {
    ...
    const uniqueUserSessionList = userSessionList.filter(
  ...
  const sendMessage = async (messageType: string) => {
    ...
    if (messageType == 'applyfile') {
      ...
      if (['png', 'jpg', 'jpeg', 'gif', 'bmp'].includes(fileExtension)) {
        ...
        ['txt', 'doc', 'pdf', 'xls', 'xlsx', 'ppt', 'pptx'].includes(
          fileExtension
        )
      ) {
  ...
  const sendImageMessage = async () => {
    ...
    if (
      ...
      Object.keys(targetGroupInfo.value).length > 0
    ) {
      // console.log('群聊发送文件');
      Message.type = 'group'
      ...
      Object.keys(targetUserInfo.value).length > 0
    ) {
      // console.log('单聊发送文件');
      Message.type = 'user'
      Message.content_type = 'image'
    ...
    // 发送文件消息
    const res = await post('user/chat/send', Message)
    if (!res.success) {
    ...
    const messageHistory = {
      ...
      chatId:
        Message.type === 'group'
          ? targetGroupInfo.value.chatId
          : targetUserInfo.value.chatId,
  ...
  const sendFileMessage = async () => {
    ...
    const messageHistory = {
      ...
      file_size: res.data.file_info?.size || 0,
      previewMessage: '[文件消息]',
      time: Date.now(),
  ...
  const updateGroupSessionList = async (message: string) => {
    const groupSessionList = await db.getByField(
      'groupSessionList',
      'chatId',
      targetChatId.value
    )
  ...
  const updateUserSessionList = async (message: string) => {
    // 更新sessionlist表的字段
    const sessionList = await db.getByField(
  ...
  const changeChatListAndChatHistory = async (data: any, message: string) => {
    ...
    const conversation = await db.getByField(
      'workbenchSessionList',
      'chatId',
      data.userId
    )
  ...
  const addChatListAndGetChatHistory = async (chatId: string, type: string) => {
    const chatIdSet = new Set(
      chatList.value.map((chat: { chatId: any }) => chat.chatId)
    )
    ...
    if (chatIdSet.has(chatId)) {
      ...
      if (type == 'group') {
        getInviteUserList()
        chatHistory.value = await db.getByField(
          'workbenchGroupChatRecord',
          'chatId',
          chatId
        )
        messageSendStatus.value = true
  ...
  const getAllUser = async () => {
    ...
    // 过滤掉当前用户
    allUserList.value = users.filter(
      (user: { id: any }) => user.id !== userInfo.value.id
    )
  }
  ...
  const createGroupChat = async (userIds: string[]) => {
    ...
    const inviteUserName = userList
      .map((user: { nickname: string }) => user.nickname)
      .join(',')
  ...
  const userChatMessage = async (data: any) => {
    ...
    const isPresence = await db.getByField(
      'workbenchChatUser',
      'chatId',
      data.userId
    )
  ...
  const getGroupList = async () => {
    const res = await get('user/chat/group/list')
    if (!res.success) {
    ...
    const formattedGroups = res.data.groups.map((group: any) => {
      const groupSession = groupSessionList.find(
        (item: { chatId: string }) => item.chatId === group.id
      )
      return {
        group_id: group.id,
    ...
    for (const group of res.data.groups) {
      // 查询数据库中是否已存在该群组的成员列表
      const existingGroup = await db.getByField(
        'workbenchGroupUserList',
        'group_id',
        group.id
      )
  ...
  const onlineUserData = async (data: OnlineUserInfoType[]) => {
    ...
    const updateOnlineStatus = async (data: any[]) => {
      const updatedChats = chatList.value.map(
        (chat: { type: string; chatId: any }) => {
          if (chat.type === 'user') {
  ...
  const inviteFriend = async (groupId: string, userIds: string[]) => {
    ...
    // 邀请加入群聊
    const url = 'user/chat/group/join'
    const res = await post(url, { group_id: groupId, user_ids: userIds })
    if (!res.success) {
      return false
    }
    // console.log(await res.json())
  ...
  const getHistory = async (
    ...
    if (type === 'user') {
      messagesHistory = await db.filter(
        'workbenchChatRecord',
        (record: any) => {
          return (
      ...
      // console.log('group')
      messagesHistory = await db.getByField(
        'workbenchGroupChatRecord',
        'chatId',
        toUserId
      )
  ...
  const setTargetUserInfo = async (id: string) => {
    ...
    const userInfo = {
      ...
      toUserId: loginStore.userInfo.id,
  ...
  const groupChatMessage = async (data: any) => {
    ...
    if (data.content_type === 'image') {
      ...
      updateRecipientGroupSessionList(
    ...
    // 找到群会话列表，更新对于id的数据。
    await updateRecipientGroupSessionList(
      messageRecord.previewMessage,
      data.to_groupid
    )
  ...
  const updateRecipientGroupSessionList = async (
    message: string,
    chatId: string
  ) => {
  ...
  const quitGroup = async (group_id: string) => {
    const url = 'user/chat/group/leave'
    const res = await post(url, { group_id })
    if (!res.success) {
      return false
    }
    // console.log(await res.json())
  ...
  const getGroupMember = async (group_id: string) => {
    const member = await db.getByField(
      'workbenchGroupUserList',
      'group_id',
      group_id
    )
  ...
  const getImageSrc = async (imageMessage: string) => {
    ...
    const path = 'user/chat/image/view?path=' + imageMessage
  ...
  const groupInviteMessage = async (data: any) => {
    ...
    const groupExists = groupList.value.some(
      (group: { group_id: string }) => group.group_id === data.group_id
    )
    ...
    if (!groupExists) {
      ...
      const existingGroupIds = new Set(
        chatList.value.map((item: { chatId: string }) => item.chatId)
      )
      const newGroups = groupList.value.filter(
        (group: { chatId: string }) => !existingGroupIds.has(group.chatId)
      )
  ...
  const getGroupMemberList = async (group_id: string) => {
    ...
    const res = await get('user/chat/group/info?gid=' + group_id)
    if (!res.success) {
      return false
    }

    // 封装 avatar 和 nickname 到 groupMembers
    groupMembers.value = res.data.group.members.map((member: any) => ({
      id: member.id,
  ...
  const clearSentMessages = async () => {
    ...
    if (resSent >= 0) {
      ...
      const res: any = await db.getByField(
        'workbenchSessionList',
        'chatId',
        targetChatId.value
      )
  ...
  const clearReceivedMessages = async () => {
    ...
    const resReceived = await db.deleteByWhere(
      'workbenchChatRecord',
      whereObjReceived
    )
    if (resReceived >= 0) {
  ...
  const clearGroupMessages = async () => {
    const res = await db.deleteByField(
      'workbenchGroupChatRecord',
      'chatId',
      targetChatId.value
    )
