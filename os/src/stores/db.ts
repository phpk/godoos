...
dbInit
  ...
  .stores({
    ...
    // 模型列表
    modelslist:
      '++id,model,label,action,engine,status,params,type,isdef,info,created_at',
    // ai对话列表
    ...
    // 用户列表
    workbenchChatUser:
      '++id,ip,userName,chatId,avatar,mobile,phone,nickName,isOnline,updatedAt,createdAt',
    // 会话列表
    workbenchSessionList:
      '++id,avatar,chatId,username,nickname,userId,toUserId,previewMessage,messages,time,createdAt',
    // 聊天记录
    workbenchChatRecord:
      '++id,[toUserId+userId], [userId+toUserId],toUserId,messages,messageType,time,createdAt,userInfo',
    // 群组会话列表
    groupSessionList:
      '++id,groupId,chatId,name,message,previewMessage,avatar,createdAt',
    // 群组聊天记录
    workbenchGroupChatRecord:
      '++id,chatId,userId,to_groupid,messageType,userInfo,message,time,type,createdAt',
    // 群用户列表
    ...
    // 用户列表
    chatuser:
      '++id,ip,hostname,userName,avatar,mobile,nickName,isOnline,updatedAt,createdAt',
    chatmsg:
      '++id,toUserId,targetIp,senderInfo,reciperInfo,previewMessage,content,type,status,isRead,isMe,readAt,createdAt',
    // 文件密码箱
  ...
  .upgrade(
    (tx: {
      ...
      // 手动添加索引
      tx.groupSessionList.addIndex(
        'chatId',
        (obj: { chatId: any }) => obj.chatId
      )
      tx.workbenchSessionList.addIndex(
        'chatId',
        (obj: { chatId: any }) => obj.chatId
      )
      tx.workbenchChatRecord.addIndex(
        'toUserId',
        (obj: { toUserId: any }) => obj.toUserId
      )
      tx.systemChatRecord.addIndex(
        'chatId',
        (obj: { chatId: any }) => obj.chatId
      )
      // 添加复合索引
      tx.workbenchChatRecord.addIndex(
        '[toUserId+userId]',
        (obj: { toUserId: any; userId: any }) => [obj.toUserId, obj.userId]
      )
      tx.workbenchGroupChatRecord.addIndex(
        'chatId',
        (obj: { chatId: any }) => obj.chatId
      )
      tx.workbenchGroupUserList.addIndex(
        'group_id',
        (obj: { group_id: any }) => obj.group_id
      )
      tx.workbenchGroupInviteMessage.addIndex(
        'group_id',
        (obj: { group_id: any }) => obj.group_id
      )
    }
