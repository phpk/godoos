...
import { cateList, aiEngines, llamaQuant, chatInitConfig } from "./modelconfig"
export const useModelStore = defineStore('modelStore', () => {
  ...
  function getEngine(cpp : string) {
    return aiEngines.find((d: any) => d.cpp == cpp)
  }
  ...
  async function getModelList() {
    const ollamaEngine:any = getEngine('ollama')
    const res = await fetch(`${ollamaEngine.url}/tags`)
    //console.log(res)
    if (res.ok) {
      await resetData(res)
    }
  ...
  async function resetData(res: any) {
    ...
    if (data && data.models.length > 0) {
      ...
      await db.deleteByWhere("modelslist",{engine: "ollama"});
      await db.addAll("modelslist", saveData);
      modelList.value = saveData;
    }
  ...
  async function addModel(data: any) {
    await db.addOne("modelslist", data)
  }
  ...
  async function getList() {
    labelList.value = await getLabelList()
    console.log(JSON.stringify(labelList.value))
    await getModelList()
  ...
  async function deleteModelList(data: any) {
    ...
    const ollamaEngine:any = getEngine('ollama')
    const delUrl = ollamaEngine.url + "/delete";
  ...
  function getOllamaDownUrl(){
    const engine = getEngine('ollama')
    return `${engine?.url}/pull`
  }
  ...
  async function chatWithModel(data: any) {
    const postData:any = {
      method: "POST",
      body: JSON.stringify(data),
    };
    console.log(postData.body)
    const engine:any = getEngine(data.engine)
    if (!engine) return
    if('apikey' in engine && data.apikey) {
      postData.headers = {
    ...
    const chatUrl = getChatUrl(engine);
    // console.log(chatUrl)
  ...
  function getChatUrl(engine:any) {
    if(engine.cpp == 'ollama') {
      return `${engine.url}/chat`
    }
    else{
      return `${engine.url}/v1/chat/completions`
    }
  }
  ...
  persist: {
    key: 'modelStore',
    pick: ['downList', 'chatConfig', 'aiEngine'],
  },
})
