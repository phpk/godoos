...
export default () => {
  ...
  // 导出PPTX文件
  const exportData = (_slides: Slide[], masterOverwrite: boolean, ignoreMedia: boolean) => {
    exporting.value = true
    ...
    for (const slide of _slides) {
      ...
      for (const el of slide.elements) {
        if (el.type === 'text') {
          ...
          if (el.shadow) options.shadow = getShadowOption(el.shadow)
          pptxSlide.addShape('line' as pptxgen.ShapeType, options)
        }
  ...
  const exportBuffer = async () => {
    const pptx = exportData(slides.value, true, false)
    const buffer = await pptx.write({ outputType: 'base64' })
    const fileName = 'pptx_' + md5(buffer.toString())
    localStorage.setItem(fileName, JSON.stringify(slides.value))
    return buffer
  }
  const exportPPT = () => {
    const pptx = exportData(slides.value, true, false)
  ...
  const exportPPTX = (_slides: Slide[], masterOverwrite: boolean, ignoreMedia: boolean) => {
    const pptx = exportData(_slides, masterOverwrite, ignoreMedia)
    setTimeout(() => {
      pptx.writeFile({ fileName: `${title.value}.pptx` }).then(() => exporting.value = false).catch(() => {
        exporting.value = false
        message.error('导出失败')
