...
let _saveType = 'Save as'
function saveToLocal(blob, name) {
...
class File_save_class {
	...
	saveToOs(blob, name) {
		//console.log(name)
		//console.log(blob)
		if (_saveType == 'Save as') {
			filesaver.saveAs(blob, name)
		} else {
			const titleArr = name.split(".");
			const ext = titleArr.pop();
			...
			blobToBase64(blob).then(base64Str => {
				...
				const fileName = 'photoshop_' + md5(base64Str)
				localStorage.setItem(fileName, this.export_as_json())
				const save = {
					data: JSON.stringify({ content: base64Str, title, ext: ext }),
					type: 'exportPhoto'
				}
				//console.log(save)
