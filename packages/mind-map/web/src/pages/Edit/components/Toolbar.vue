      ...
      <div class="toolbarBlock">
        ...
        >
          ...
          <div slot="reference" class="toolbarBtn">
            <span class="icon iconfont icongongshi" title="更多"></span>
          </div>
        </el-popover>
      </div>
      <div class="toolbarBlock">
        ...
        <div class="toolbarBtn" @click="$bus.$emit('showExport')">
          <span class="icon iconfont iconexport" title="导出"></span>
        </div>
        <div class="toolbarBtn" @click="saveData">
          <span class="icon iconfont iconlingcunwei" title="保存"></span>
        </div>
      </div>
      <!-- 导出 -->
      <!-- <div class="toolbarBlock">
...
export default {
  ...
  mounted() {
    ...
    this.$bus.$on('data_change', data => {
      ...
      if (data) { 
        if(data.root){
          this.nodeData.root = {...data.root};
        }else{
          this.nodeData.root = {...data};
        }
      }
    })
  ...
  beforeDestroy() {
    ...
    document.removeEventListener('keydown', this.debouncedHandleKeyDown)
  },
  methods: {
    ...
    eventHandler(e) {
      ...
      if (eventData.type === 'start') {
        ...
        this.setData(JSON.stringify(defNode))
      }
      if (eventData.type === 'init') {
        ...
        if (data.content) {
          if (typeof data.content === 'string' && isBase64(data.content)) {
            ...
            this.setData(JSON.stringify(data.content))
          }
        } else {
          console.log(evTitle)
        }
    ...
    getTitle(data) {
      //console.log(data)
      const htmlStr = data.root.data.text
      //console.log(htmlStr)
