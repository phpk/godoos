...
import (
	"bytes"
	"fmt"
	"godocms/pkg/office/libs"
	pdf "godocms/pkg/office/pdf"
	xlsx "godocms/pkg/office/xlsx"
	"html"
	"math"
	"os"
	"regexp"
...
func pdf2txt(filename string) (string, error) { // BUG: Cannot get text from specific (or really malformed?) pages
	...
	if err != nil {
		return libs.GetPdfText(filename)
	}
...
func isSameSentence(text1, text2 pdf.Text) bool {
	// 这里可以根据具体需求实现逻辑
	// 例如：检查字体、字号、位置等是否相同
	return text1.Font == text2.Font && text1.FontSize == text2.FontSize && math.Abs(text1.Y-text2.Y) < 1e-6
}
